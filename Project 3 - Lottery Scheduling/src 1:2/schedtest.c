...
main(int argc, char *argv[])
{
  ...
  //printf(0," About to make parent sleep for [%d]\n", sleepParent);
  ...
  printf(1, "%d %d: %d\n", runticksA, runticksB, runticksA/runticksB);
    ...
    // prints number of tickets both children had
    int runticketsA = -1, runticketsB = -1;
    for (int i = 0; i < NPROC; ++i) {
      if (pstat.pid[i] == pidA)
        runticketsA = pstat.tickets[i];
      if (pstat.pid[i] == pidB)
        runticketsB = pstat.tickets[i];
    }
    printf(1, "%d %d \n", runticketsA, runticketsB);
